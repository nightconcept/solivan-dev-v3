...
import { calculateReadTime, getValidBlogPosts, stripMarkdown, truncateDescription } from "./utils";
  ...
  describe("getValidBlogPosts", () => {
    ...
    it("should handle undefined or array author", async () => {
      (getCollection as Mock).mockResolvedValue([
        ...
        {
          id: "post2",
          data: { title: "Post 2", date: new Date("2023-01-02"), author: ["Author A", "Author B"] },
          body: "body2",
        },
    ...
    it("should handle undefined title, description, draft, and body", async () => {
      (getCollection as Mock).mockResolvedValue([{ id: "post1", data: { date: new Date() }, body: undefined }]);
      const posts = await getValidBlogPosts();
      expect(posts.length).toBe(1);
